/// budget.zig - Memory budget tracking for wasmz
///
...
///
/// Placed in a standalone file to avoid circular imports between store.zig
/// (which owns GcHeap) and vm/gc/heap.zig (which needs to update the budget).
